...
//192.168.1.5
//192.168.43.243
const imageBaseUrl = 'http://192.168.1.106:8000/';
class ApiService {
  //http://127.0.0.1:8000/api

  final _baseUrl = 'http://192.168.1.106:8000/api/';
  final Dio _dio;
  ...
  Future<Map<String, dynamic>> post({required String endPoint, required Map<String,dynamic> query,String? token}) async {
    var response = await _dio.post('$_baseUrl$endPoint',queryParameters:query,options: Options(
        headers:{'Accept':'application/json','Content-Type':'application/json','Authorization':'Bearer$token'} ,
    receiveDataWhenStatusError: true,
   sendTimeout:const Duration(minutes:  60*1000) , // 60 seconds
  ...
  Future<Map<String, dynamic>> delete({required String endPoint, required Map<String,dynamic> query,String? token}) async {
    var response = await _dio.delete('$_baseUrl$endPoint',queryParameters:query,options: Options(
        headers:{'Accept':'application/json','Content-Type':'application/json','Authorization':'Bearer$token'} ,
        receiveDataWhenStatusError: true,
        sendTimeout:const Duration(minutes:  60*1000) , // 60 seconds
        receiveTimeout: const Duration(minutes: 60*1000) ),);// 60 seconds) );
